...
const API_URL = process.env.NEXT_PUBLIC_API_URL || "https://easypips-engine.onrender.com";
  ...
  async function loadChart(market: string) {
    ...
    try {
      const res = await fetch(
        `${API_URL}/candles?market=${encodeURIComponent(market)}&interval=5m&limit=180`,
        { cache: "no-store" }
      );
        ...
        <section className="grid gap-4 lg:grid-cols-[300px_minmax(0,1fr)_340px]">
          <div className="space-y-4">
            <Panel tint="blue" title="Active and Pending Orders">
              <div className="space-y-5">
                <div>
                  <p className="mb-3 text-xs uppercase tracking-[0.22em] text-blue-200/70">Active</p>
                ...
                <div>
                  <p className="mb-3 text-xs uppercase tracking-[0.22em] text-blue-200/70">Pending</p>
                  <div className="space-y-2">
          ...
          <div className="space-y-4">
            <Panel tint="yellow" title="Selected Signal">
              <div className="flex flex-wrap items-center gap-3">
                ...
                <Badge tone={side === "LONG" ? "green" : side === "SHORT" ? "red" : "muted"}>
            ...
            <Panel tint="purple" title="Trade History">
              <div className="space-y-2">
                {closed.length ? closed.slice(0, 6).map((s, i) => (
          ...
          <div className="space-y-4">
            <Panel tint="red" title="Desk 1 and Desk 2">
              <div className="space-y-4">
                <DeskCard title="Desk 1" trader="Doctor Rano" about="Senior trader and market analyst." pair={selectedPair} signal={desk1Signal} />
                <DeskCard title="Desk 2" trader="Doctor Fahdi" about="Execution focused trader and signal supervisor." pair={selectedPair} signal={desk2Signal} />
              </div>
            </Panel>
...
function Panel({
  ...
  const classes = {
    yellow: "border-yellow-400/25 bg-[linear-gradient(180deg,rgba(250,204,21,0.10),rgba(12,23,41,0.96))]",
    blue: "border-blue-400/25 bg-[linear-gradient(180deg,rgba(59,130,246,0.10),rgba(12,23,41,0.96))]",
    green: "border-emerald-400/25 bg-[linear-gradient(180deg,rgba(16,185,129,0.10),rgba(12,23,41,0.96))]",
    purple: "border-fuchsia-400/20 bg-[linear-gradient(180deg,rgba(168,85,247,0.10),rgba(12,23,41,0.96))]",
    lightblue: "border-sky-300/25 bg-[linear-gradient(180deg,rgba(56,189,248,0.10),rgba(12,23,41,0.96))]",
    red: "border-rose-400/25 bg-[linear-gradient(180deg,rgba(244,63,94,0.10),rgba(12,23,41,0.96))]",
  }[tint];
...
function ChartBox({
  ...
  useEffect(() => {
    ...
    const chart = createChart(ref.current, {
      ...
      height: 460,
      layout: { background: { color: "#091425" }, textColor: "#8fa3bf", attributionLogo: false },
  ...
  useEffect(() => {
    ...
    const data = candles
      .map((c) => {
        ...
        return { time: Math.floor(t / 1000) as UTCTimestamp, open: c.open, high: c.high, low: c.low, close: c.close };
      })
      .filter(Boolean) as { time: UTCTimestamp; open: number; high: number; low: number; close: number }[];
    series.setData(data);
    ...
    const add = (price: number | undefined, color: string, title: string) => {
      ...
      const line = series.createPriceLine({ price, color, lineWidth: 2, lineStyle: LineStyle.Dashed, axisLabelVisible: true, title });
  ...
  if (loading) return <div className="grid h-[460px] place-items-center rounded-[24px] border border-white/10 bg-[#091425] text-slate-400">Loading chart...</div>;
  if (!candles.length) return <div className="grid h-[460px] place-items-center rounded-[24px] border border-white/10 bg-[#091425] text-slate-400">No chart data.</div>;
...
function SignalListItem({
  ...
  return (
    <button
      onClick={onClick}
      className={`w-full rounded-2xl border px-4 py-3 text-left ${selected ? "border-blue-300/35 bg-blue-300/10" : "border-white/10 bg-[#0f1c31]"}`}
    >
      <div className="flex items-center justify-between gap-3">
        <div>
          <p className="text-sm font-bold text-white">{signal.market || "-"}</p>
          <p className="text-xs text-slate-400">{signal.display_decision || signal.decision || "WAIT"}</p>
        </div>
        <span className={`rounded-full px-2 py-1 text-[10px] font-bold uppercase ${side === "LONG" ? "bg-green-400/15 text-green-300" : side === "SHORT" ? "bg-red-400/15 text-red-300" : "bg-slate-500/15 text-slate-300"}`}>
          {side}
        </span>
      </div>
    </button>
...
function DeskCard({
  ...
  about: string;
  pair: string;
  signal: { side: "LONG" | "SHORT"; entry?: number; stopLoss?: number; target?: number; note: string; time: string };
}) {
  return (
    <div className="rounded-[24px] border border-rose-300/20 bg-[#0f1c31] p-4">
      <div className="flex items-start justify-between gap-3">
        ...
        <span className="rounded-full border border-rose-300/20 bg-rose-300/10 px-3 py-1.5 text-xs font-bold uppercase text-rose-100">Live</span>
      </div>
    ...
    <div className="rounded-2xl border border-white/10 bg-[#0f1c31] px-4 py-3">
      <div className="flex items-center justify-between gap-3">
        ...
        <div className="text-right">
          ...
          <p className="text-xs text-slate-500">{signal.published_at ? new Date(signal.published_at).toLocaleString() : "-"}</p>
        </div>
      </div>
    </div>
  );
...
function Chip({ children }: { children: React.ReactNode }) {
  return <span className="rounded-full border border-white/10 bg-white/5 px-3 py-1.5">{children}</span>;
}

function Badge({ children, tone = "muted" }: { children: React.ReactNode; tone?: "muted" | "green" | "red" }) {
  const cls =
    tone === "green"
...
function EmptyInline({ text }: { text: string }) {
  return <div className="rounded-2xl border border-white/10 bg-[#0f1c31] px-4 py-4 text-sm text-slate-400">{text}</div>;
}
...
function Stat({ label, value, tone }: { label: string; value: string; tone?: "red" | "green" }) {
  return (
    <div className="rounded-2xl border border-white/10 bg-[#0f1c31] px-4 py-3">
      ...
      <p className={`mt-2 text-sm font-bold ${tone === "green" ? "text-green-400" : tone === "red" ? "text-red-400" : "text-white"}`}>{value}</p>
    </div>
  );
}
...
function buildDeskSignal(signal: SignalLike | null, desk: "RANO" | "FAHDI") {
  ...
  return {
    ...
    note: desk === "RANO" ? "Patient entry, disciplined stop, and one clean target." : "Daily tactical execution with one entry, one stop, and one target.",
    time: signal?.published_at ? new Date(signal.published_at).toLocaleString() : new Date().toLocaleString(),
  };
}
